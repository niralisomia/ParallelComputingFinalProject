...
# Detect cell type key (matches what Julia notebook uses)
...
# ── Preprocessing (shared between SS and dyn) ─────────────────────────────────
print("\nPreprocessing...")
...
# ── Helper: per-cell-type velocity consistency ────────────────────────────────
def compute_consistency(adata_fit, cell_type_key):
    ...
    for ct in unique:
        ...
        if len(idx) == 0:
            continue
        T_sub = T[idx]           # rows for this cell type
        total  = T_sub.sum()
        if total < 1e-12:
            continue
...
scv.tl.velocity_graph(adata_ss)
...
n_vel_genes_ss = int((~np.isnan(adata_ss.layers["velocity"][:, 0])).sum())
print(f"  Median confidence: {ss_conf:.3f}")
print(f"  Consistency: { {k: round(v, 3) for k, v in ss_consistency.items()} }")

# ── Dynamical model (optional — slow) ─────────────────────────────────────────
dyn_time        = None
dyn_conf        = None
dyn_consistency = {}
n_vel_genes_dyn = None
...
if args.dyn:
    ...
    n_vel_genes_dyn = int((~np.isnan(adata_dyn.layers["velocity"][:, 0])).sum())
...
print("="*55)
print("Next: run the Julia benchmark cell in the notebook to generate comparison plots.")
